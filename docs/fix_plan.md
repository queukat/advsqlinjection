...
| A10 | Compatibility / verification discipline is weak | Medium | Open-ended compatibility and limited verification discipline | stale / already fixed | Compatibility has a concrete `since-build`; no explicit upper bound is set; verifier task was run successfully |
| A11 | Searchable options / settings discoverability are weak | Medium | Searchable options were incomplete and settings UX was not standard | fixed | Removed `nonDefaultProject`, so generated searchable options now include the plugin settings configurable |
...
- Status: done
  ...
  Fix: Replaced the raw list with a managed rules table, add/edit/delete/reorder flows, validation dialog, example rule, bundled setup guide opener, and real preview against the current file.
...
- Status: done
  ...
  Fix: Renamed the product surface to `Advanced Language Injection`, updated plugin metadata, added a repository README, declared a concrete `since-build`, removed the explicit upper build bound, and refreshed release notes.
...
- Status: done
  ...
  Fix: Reworked the settings UI, implemented `SearchableConfigurable`, removed `nonDefaultProject` from the project configurable, and re-ran searchable-options generation.
  Files: `src/main/kotlin/com/queukat/advsqlinjection/settings/AdvancedSQLInjectionSettingsConfigurable.kt`, `src/main/kotlin/com/queukat/advsqlinjection/settings/AdvancedSQLInjectionSettingsPanel.kt`, `src/main/resources/META-INF/plugin.xml`
  Verification: `buildSearchableOptions --rerun-tasks` completes, and the generated plugin searchable-options XML now contains `com.queukat.advsqlinjection.settings`.
  Notes: The settings remain project-scoped; the configurable is now also visible to the default-project traversal used by searchable-options generation.
- Status: done
...
- Re-ran searchable-options generation as part of packaging/verifier flows; generation succeeds, and plugin settings entries are present.
